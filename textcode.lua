...
-- FIX 1: Khai báo biến thiếu
PointStats = 1  -- Cho Auto Stats
MousePos = Vector3.new(0, 0, 0)  -- Cho Aimbot
Pos = CFrame.new(0, 0, 0)  -- Cho MaterialMon
TweenSpeed = 350
...
-- FIX 4: Thêm hàm GetFirePlacard
function GetFirePlacard(num, direction)
    local CommF_ = game:GetService("ReplicatedStorage").Remotes.CommF_
    CommF_:InvokeServer("GuitarPuzzleProgress", "Gravestones", num, direction)
end

-- FIX 5: Sửa hàm CheckQuest (giữ nguyên như code gốc)
function CheckQuest() 
...
local function weaponSc(weapon)
    ...
    -- Tìm trong Character (vũ khí đang cầm)
...
function BringEnemy()
    ...
    if not PosMon then return end  -- <--- DÒNG MỚI
    local player = game.Players.LocalPlayer
    local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    local speed = getgenv().BringMode or 250
