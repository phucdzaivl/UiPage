-- ==================== DRAGON HUB SIMPLE ====================
-- Chỉ Auto Farm Level - Không lỗi đánh không dame
...
function CheckQuest()
    ...
    if World1 then
        ...
        elseif MyLevel >= 375 and MyLevel <= 399 then
            ...
            if (CFrameQuest.Position - plr.Character.HumanoidRootPart.Position).Magnitude > 10000 then
    ...
    elseif World2 then
        ...
        elseif MyLevel >= 1350 and MyLevel <= 1374 then
            ...
            if (CFrameQuest.Position - plr.Character.HumanoidRootPart.Position).Magnitude > 10000 then
                replicated.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-6508.5581054688, 5000.034996032715, -132.83953857422))
            end
...
function SelectWeapon()
    ...
    -- Fallback: equip bất kỳ tool nào
    for _, v in pairs(plr.Backpack:GetChildren()) do
        if v:IsA("Tool") then
            EquipWeapon(v.Name)
            return v.Name
...
function AttackMonster(monster)
    ...
    -- Đứng gần monster
    local attackPos = monster.HumanoidRootPart.CFrame * CFrame.new(0, 25, 0)
...
spawn(function()
    while wait() do
        if getgenv().AutoFarm then
            pcall(function()
                -- Auto Buso Haki
                if getgenv().AutoHakiBuso and not plr.Character:FindFirstChild("HasBuso") then
                ...
                local questText = questUI.Visible and questUI.Container.QuestTitle.Title.Text or ""
                
                -- Nếu chưa có quest
                if not questUI.Visible then
                    topos(CFrameQuest)
                    task.wait(0.5)
                    if (CFrameQuest.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 30 then
                    ...
                    -- Nếu có monster trong workspace
                    if monster then
                        -- Chọn weapon
                        SelectWeapon()
                        ...
                        -- Nếu monster trong ReplicatedStorage
                        local monsterInRep = replicated:FindFirstChild(Mon)
                        if monsterInRep and monsterInRep:FindFirstChild("HumanoidRootPart") then
                            topos(monsterInRep.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                        else
                            -- Tìm trong EnemySpawns
...
Title.Text = "🔥 DRAGON HUB - AUTO FARM"
...
FarmBtn.Position = UDim2.new(0.05, 0, 0.22, 0)
FarmBtn.Size = UDim2.new(0.4, 0, 0.12, 0)
...
FarmBtn.MouseButton1Click:Connect(function()
    ...
    FarmBtn.BackgroundColor3 = getgenv().AutoFarm and Color3.fromRGB(200, 40, 40) or Color3.fromRGB(0, 180, 0)
    FarmBtn.Text = getgenv().AutoFarm and "⏹ STOP FARM" or "▶ START FARM"
end)

-- Bring Mob Button
local BringBtn = Instance.new("TextButton")
BringBtn.Parent = Content
BringBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 0)
BringBtn.BorderSizePixel = 0
BringBtn.Position = UDim2.new(0.55, 0, 0.22, 0)
BringBtn.Size = UDim2.new(0.4, 0, 0.12, 0)
BringBtn.Font = Enum.Font.GothamBold
BringBtn.Text = "🔰 BRING MOB: ON"
BringBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
BringBtn.TextSize = 18

BringBtn.MouseButton1Click:Connect(function()
    getgenv().BringMob = not getgenv().BringMob
    BringBtn.BackgroundColor3 = getgenv().BringMob and Color3.fromRGB(0, 180, 0) or Color3.fromRGB(200, 40, 40)
    BringBtn.Text = getgenv().BringMob and "🔰 BRING MOB: ON" or "🔰 BRING MOB: OFF"
end)
...
WeaponLabel.Position = UDim2.new(0.05, 0, 0.36, 0)
WeaponLabel.Size = UDim2.new(0.3, 0, 0.08, 0)
...
WeaponDropdown.BackgroundColor3 = Color3.fromRGB(40, 40, 70)
WeaponDropdown.BorderSizePixel = 1
WeaponDropdown.BorderColor3 = Color3.fromRGB(100, 100, 150)
WeaponDropdown.Position = UDim2.new(0.35, 0, 0.36, 0)
WeaponDropdown.Size = UDim2.new(0.25, 0, 0.08, 0)
WeaponDropdown.Font = Enum.Font.GothamSemibold
WeaponDropdown.Text = "Melee"
WeaponDropdown.TextColor3 = Color3.fromRGB(255, 255, 255)
WeaponDropdown.TextSize = 16
...
WeaponDropdown.MouseButton1Click:Connect(function()
    ...
    getgenv().SelectWeapon = weaponOptions[weaponIndex]
    WeaponDropdown.Text = getgenv().SelectWeapon
end)

-- Auto Buso Button
local BusoBtn = Instance.new("TextButton")
BusoBtn.Parent = Content
BusoBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 0)
BusoBtn.BorderSizePixel = 0
BusoBtn.Position = UDim2.new(0.65, 0, 0.36, 0)
BusoBtn.Size = UDim2.new(0.3, 0, 0.08, 0)
BusoBtn.Font = Enum.Font.GothamBold
BusoBtn.Text = "🛡 BUSO: ON"
BusoBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
BusoBtn.TextSize = 16

BusoBtn.MouseButton1Click:Connect(function()
    getgenv().AutoHakiBuso = not getgenv().AutoHakiBuso
    BusoBtn.BackgroundColor3 = getgenv().AutoHakiBuso and Color3.fromRGB(0, 180, 0) or Color3.fromRGB(200, 40, 40)
    BusoBtn.Text = getgenv().AutoHakiBuso and "🛡 BUSO: ON" or "🛡 BUSO: OFF"
end)
...
spawn(function()
    while wait(1) do
        pcall(function()
            local lv = plr.Data.Level.Value or 0
            local beli = plr.Data.Beli.Value or 0
            local frag = plr.Data.Fragments.Value or 0
            StatusLabel.Text = string.format("📊 STATUS\nLevel: %s | Beli: %s | Fragments: %s\n\n⚔️ Weapon: %s | 🔰 Bring Mob: %s", 
                lv, beli, frag, getgenv().SelectWeapon, getgenv().BringMob and "ON" or "OFF")
        end)
    end
end)

print("✅ DRAGON HUB - AUTO FARM ĐÃ SẴN SÀNG!")
print("🔥 Click START FARM để bắt đầu!")
