...
pcall(function()
    ...
    if not setreadonly then setreadonly = function(obj, val) end end
    if not newcclosure then newcclosure = function(func) return func end end
    if not sethiddenproperty then sethiddenproperty = function(obj, prop, val) end end
    if not settings then settings = function() return { Rendering = { QualityLevel = "Level01" } } end end
    if not setclipboard then setclipboard = function(text) print("📋 "..text) end end
    if not syn then syn = { request = function() end } end
...
function getTrophies(index)
    local data = {{true,-720},{true,-720},{true,-720},{true,-180},{true,-180},{true,-180}}
    return data[index] or {false,0}
end

function GetFirePlacard(num, direction)
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("GuitarPuzzleProgress", "Gravestones", num, direction)
end
            ...
            for AttributeName, Berry in pairs(Bush:GetAttributes()) do
                if Berry then
                    ...
                    if Bush.Parent:FindFirstChild("BerryESP") then
                        local pos = (Bush.Parent:GetPivot().Position - head.Position).Magnitude
            ...
            if IslandESP then
                if v.Name ~= "Sea" then
                    if not v:FindFirstChild("IslandESP") then
                        ...
                        local dist = (v.Position - head.Position).Magnitude / 3
    ...
    for _, v in pairs(game.Workspace:GetChildren()) do
        pcall(function()
            if DevilFruitESP and string.find(v.Name, "Fruit") and v:FindFirstChild("Handle") then
                ...
                local espName = "FruitESP_" .. ESPNumber
                ...
                if handle:FindFirstChild(espName) then
                    local dist = (handle.Position - head.Position).Magnitude / 3
                    handle[espName].TextLabel.Text = v.Name .. "\n" .. math.floor(dist) .. "m"
                end
            else
                if v:FindFirstChild("Handle") then
                    for _, child in pairs(v.Handle:GetChildren()) do
                        if string.find(child.Name, "FruitESP_") then
                            child:Destroy()
                        end
                    end
                end
    ...
    for _, v in pairs(game:GetService("Players"):GetPlayers()) do
        pcall(function()
            if v ~= player and v.Character and v.Character:FindFirstChild("Head") then
                local vHead = v.Character.Head
                local espName = "PlayerESP_" .. ESPNumber
                if ESPPlayer then
                    if not vHead:FindFirstChild(espName) then
                        local bill = Instance.new("BillboardGui", vHead)
                        bill.Name = espName
                        bill.ExtentsOffset = Vector3.new(0, 1, 0)
                        bill.Size = UDim2.new(1, 200, 1, 30)
                        bill.Adornee = vHead
                    ...
                    if vHead:FindFirstChild(espName) then
                        local health = v.Character:FindFirstChild("Humanoid")
                        local dist = (vHead.Position - head.Position).Magnitude / 3
                        if health then
                            local hpPercent = math.floor(health.Health * 100 / health.MaxHealth)
                            vHead[espName].TextLabel.Text = v.Name .. " | " .. math.floor(dist) .. "m\nHealth: " .. hpPercent .. "%"
                        end
                    end
                else
                    if vHead:FindFirstChild(espName) then
                        vHead[espName]:Destroy()
                    end
                end
            end
        end)
...
GetConnectionEnemies = function(a)
    local function findInFolder(folder)
        ...
        for _, v in pairs(folder:GetChildren()) do
            if v:IsA("Model") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") then
                if typeof(a) == "table" and table.find(a, v.Name) then
                    return v
    ...
    local enemy = findInFolder(workspace:FindFirstChild("Enemies"))
    if enemy then return enemy end
    enemy = findInFolder(ReplicatedStorage)
    if enemy then return enemy end
    enemy = findInFolder(workspace)
    if enemy then return enemy end
    return nil
end
...
Attack.Kill2 = function(model, Succes)
    if model and Succes then
        ...
        if RandomCFrame then
            wait(0.1); topos(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25))
            wait(0.1); topos(model.HumanoidRootPart.CFrame * CFrame.new(25, 30, 0))
            wait(0.1); topos(model.HumanoidRootPart.CFrame * CFrame.new(-25, 30, 0))
            wait(0.1); topos(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25))
            wait(0.1); topos(model.HumanoidRootPart.CFrame * CFrame.new(-25, 30, 0))
        end
...
function BringEnemy()
    ...
    local speed = getgenv().BringModeSpeed or 250
    for _, v in pairs(workspace.Enemies:GetChildren()) do
        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
            if (v.HumanoidRootPart.Position - PosMon).Magnitude <= 1500 then
                v.HumanoidRootPart.Velocity = (root.Position - v.HumanoidRootPart.Position).Unit * speed
...
-- ==================== UI CHO ARCEUS X ====================
print("✅ Dragon Hub BETA - Đang tải UI...")

-- Sử dụng UI đơn giản cho Arceus X
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/ArceusX/UI/main/Ui.lua"))()

if not Library then
    -- Fallback: Dùng UI cũ nếu không tải được
    Library = loadstring(game:HttpGet("https://luacrack.site/index.php/Luongdaynetroi/raw/DacCauHub/Ui"))()
end

if Library then
    print("✅ Dragon Hub BETA - UI đã tải thành công!")
    -- Tạo UI...
    local Window = Library:MakeWin({
        Title = "Dragon Hub [ BETA ]",
        SubTitle = "by DucLuong - Arceus X",
        SaveFolder = "Dragon Hub | Blox Fruits.lua"
    })
...
LocalPlayer:AddTextBox({
    Name = "Số điểm muốn cộng mỗi lần",
    Placeholder = "Nhập số (VD: 1, 2, 3...)",
    ClearOnFocus = true,
    Callback = function(Value)
        PointStats = tonumber(Value) or 1
    end
})
-- ===========================================
                    ...
                    for _, v in pairs(enemies:GetChildren()) do
                        if v:IsA("Model") and (v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or v.Name == "Demonic Soul" or v.Name == "Posessed Mummy") then
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                repeat
                                    task.wait()
                                    Attack.Kill(v, getgenv().QuestTrain_2)
                ...
                if boss then
                    for _, v in pairs(enemies:GetChildren()) do
                        ...
                        then
                            ...
                            topos(v.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                            if syn and syn.request and not getgenv().SimulationSet then
...
-- FIX: Nếu danh sách rỗng, thêm placeholder
if #PlrList == 0 then
    table.insert(PlrList, "No Players")
end
Player:AddDropdown({
